...
final class payapiSettings
{
    ...
    private static function isolated()
    {
        return md5(
            str_replace(
                '*',
                'store',
                ((getenv('HTTP_HOST', true) !== false) ? getenv('HTTP_HOST', true) : getenv('HTTP_HOST'))
            )
        );
    }
    ...
    {
        if (self::get('sandbox') == 1 || (is_string(self::get('___public')) === true &&
            self::get('___public') != null)) {
            return true;
        }
        return false;
    }
    ...
    public function partials()
    {
        if (is_array(self::merchant()) === true) {
            if (isset(self::$settings[self::isolated()]['merchant']['partialPayments']) === true &&
                is_array(self::$settings[self::isolated()]['merchant']['partialPayments'])) {
